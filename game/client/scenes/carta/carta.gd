extends Sprite2D
## Representación visual de una carta: la pinta y gestiona hover/arrastre.
## No decide nada del juego; solo muestra la [Carta] que le asignen.

const RUTA_TEXTURAS := "res://client/assets/cartas/%s.png"
const TEXTURA_REVERSO := preload("res://client/assets/cartas/reverso.png")

## Carta que se muestra. [code]null[/code] = boca abajo (p. ej. cartas del rival,
## que el servidor nunca envía; ver vista_para_jugador en analisis_funcional_app.md §7).
var carta: Carta = null:
	set(valor_nuevo):
		carta = valor_nuevo
		texture = textura_para(carta)
...
static func textura_para(p_carta: Carta) -> Texture2D:
	if p_carta == null:
		return TEXTURA_REVERSO
	return load(RUTA_TEXTURAS % p_carta.id)
