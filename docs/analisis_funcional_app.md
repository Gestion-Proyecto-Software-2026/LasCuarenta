...
2. **Superar los 100 tantos.** Si al terminar las idas solo una pareja supera los 100, esa pareja gana la partida. Si **ambas** superan los 100, gana la pareja que hizo la última baza de las idas, **siempre que esa pareja haya sumado al menos 30 tantos solo por valor de cartas (sin contar cantes)**. ⚠️ El reglamento no especifica qué ocurre si esa condición de los 30 tantos no se cumple — **queda como decisión de equipo pendiente** (candidatas razonables: tratarlo como si nadie hubiera llegado a 100 y pasar a vueltas, o dar la partida directamente a la otra pareja; hay que decidirlo antes de programar `calcular_resultado` para este caso).
...
- **Decisión de equipo pendiente:** §8 tiene ahora un caso sin resolver — el desempate cuando ambas parejas superan 100 tantos pero la pareja que ganó la última baza no llega a los 30 tantos de cartas exigidos. Hay que decidirlo antes de programar `calcular_resultado` para Guiñote.
